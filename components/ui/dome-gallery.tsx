...
type DomeGalleryProps = {
  ...
  grayscale?: boolean;
  // Adds additional rows at the top and bottom of the gallery
  extraRowsTopBottom?: number;
  // Applies a compression factor to spacing of the added rows
  extraRowsSpacingFactor?: number;
  // Scales tile width for added rows to increase side-to-side gap
  extraRowsSizeXFactor?: number;
};
...
function buildItems(
  pool: ImageItem[],
  seg: number,
  extraRowsTopBottom: number = 0,
  extraRowsSpacingFactor: number = 1,
  extraRowsSizeXFactor: number = 1
): ItemDef[] {
  const xCols = Array.from({ length: seg }, (_, i) => -37 + i * 2);

  const makeYsForColumn = (colIndex: number) => {
    const isEven = colIndex % 2 === 0;
    const baseStart = isEven ? -4 : -3;
    const baseEnd = isEven ? 4 : 5;
    const start = baseStart - extraRowsTopBottom * 2;
    const end = baseEnd + extraRowsTopBottom * 2;
    const ys: number[] = [];
    for (let y = start; y <= end; y += 2) {
      let yAdj = y;
      if (extraRowsTopBottom > 0 && extraRowsSpacingFactor !== 1) {
        if (y < baseStart) {
          yAdj = baseStart + (y - baseStart) * extraRowsSpacingFactor;
        } else if (y > baseEnd) {
          yAdj = baseEnd + (y - baseEnd) * extraRowsSpacingFactor;
        }
      }
      ys.push(yAdj);
    }
  ...
  const coords = xCols.flatMap((x, c) => {
    const ys = makeYsForColumn(c);
    const isEven = c % 2 === 0;
    const baseStart = isEven ? -4 : -3;
    const baseEnd = isEven ? 4 : 5;
    return ys.map((y) => {
      const isExtraRow = y < baseStart || y > baseEnd;
      const sizeX = isExtraRow ? 2 * extraRowsSizeXFactor : 2;
      return { x, y, sizeX, sizeY: 2 };
    });
  });
...
export default function DomeGallery({
  ...
  grayscale = true,
  extraRowsTopBottom = 0,
  extraRowsSpacingFactor = 0.85,
  extraRowsSizeXFactor = 0.85,
}: DomeGalleryProps) {
  ...
  const items = useMemo(
    () =>
      buildItems(
        ...
        segments,
        extraRowsTopBottom,
        extraRowsSpacingFactor,
        extraRowsSizeXFactor
      ),
    [
      images,
      segments,
